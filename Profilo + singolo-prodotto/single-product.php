...
if(isset($_GET['id'])){
    ...
    if(isset($_GET['titolo'])&& isset($_GET['testo'])){
        $titolo = $_GET['titolo'];
        $testo = $_GET['testo'];
         $db->query("insert into recensioni (`titolo`,`testo`,`username`,`id_prodotto`) VALUE ('$titolo','$testo','tom', $var) ");
        }
    ...
    $db->query("SELECT * FROM immagini join prodotti on immagini.id_immagine = prodotti.id_immaginePrincipale WHERE prodotti.id_prodotto= $var");
