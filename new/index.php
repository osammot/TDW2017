...
$db->query("SELECT prodotti.*, immagini.path FROM prodotti left join immagini on prodotti.id_immaginePrincipale= immagini.id_immagine LIMIT 20"); //per div alcuni dei nostri prodotti
...
$db->query("SELECT prodotti.*, immagini.path FROM prodotti left join immagini on prodotti.id_immaginePrincipale= immagini.id_immagine WHERE prezzo <= 20.00 LIMIT 3"); //div a prezzi bassi
...
foreach($row3 as $rows3) {
    ...
    $body->setContent("titolo",$vartitolo3);
